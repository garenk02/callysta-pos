...
export async function getUsers(): Promise<{ users: User[] | null; error: Error | null }> {
  try {
    // Check environment variables
    const envCheck = checkSupabaseEnv();
    ...
    if (error) {
      ...
      if (error.code === '42P01') {
        return {
          users: null,
          error: new Error("The 'profiles' table doesn't exist in your Supabase database. Please create it first.")
        };
      }
    ...
    const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred fetching users';
    return { users: null, error: new Error(errorMessage) };
  }
}
